...
RegisterNetEvent("qb-gangmenu:server:withdrawMoney", function(amount)
	...
	if RemoveGangMoney(gang, amount) then
		...
		TriggerEvent('qb-log:server:CreateLog', src, 'gangmenu', 'Withdraw Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully withdrew $' .. amount .. ' (' .. gang .. ')', false)
...
RegisterNetEvent("qb-gangmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerEvent('qb-log:server:CreateLog', src, 'gangmenu', 'Deposit Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully deposited $' .. amount .. ' (' .. gang .. ')', false)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetGang("none", '0') then
				TriggerEvent('qb-log:server:CreateLog', src, "gangmenu", "Gang Fire", "orange", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.gang.name .. ")", false)
...
RegisterNetEvent('qb-gangmenu:server:HireMember', function(recruit)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		...
		TriggerEvent('qb-log:server:CreateLog', src, 'gangmenu', 'Recruit', 'yellow', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
	end
	TriggerClientEvent('qb-gangmenu:client:OpenMenu', src)
end)
